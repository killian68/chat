Informations complémentaires Chat V3


1 Précisions sur le programme.

	1.1 Finalité.
	Ce programme est un outil de messagerie dédie aux associations
...
2. Outils employés pour le développement de ce programme.
	...
	Ils sont la propriété de l'association pour le logo de la fenêre de login, le reste provenant de la bibliothèque d'icones gratuite FatCow Web Hosting free icons library
	Un autre logo peut bien sur etre substitué. Afin d'éviter de devoir modifier le code, vous pouvez lui donner le même nom que le logo en cours. Sa taille doit etre de 340*338 pixels sauvegardés au foramat png


3. Manuel d'utilisation.

	3.1 Téléchargement.
	Le programme peut être téléchargé depuis le repository gitHub
...
3.4.4. La fenêtre de configuration.
		...
		Depuis cette fenêtre, vous pouvez choisir votre méthode de connexion à la base de données qui stocke les messages. Si vous êtes connectés à internet depuis le réseau local de l'auteur du programme, il vous faut choisir «  Adressi IP locale du serveur MySQL  ». Si vous êtes dans n'importe quelle autre situation, il vous faut choisir «  Hostname distant du serveur MySQL  ».
